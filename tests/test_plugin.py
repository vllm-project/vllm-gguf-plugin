...
def test_gguf_linear_replicated_direct_store(monkeypatch):
    ...
    loader(layer.weight, torch.ones((4, 4), dtype=torch.uint8))
    assert layer.weight.data.shape == (4, 4)

    type_loader = layer.weight_type.weight_loader
    type_loader(layer.weight_type, torch.tensor(3, dtype=torch.uint8))
    assert layer.weight_type.weight_type == 3
