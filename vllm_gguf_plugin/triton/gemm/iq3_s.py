...
def iq3_s_gemm_kernel(
    ...
    for kb in range(0, num_k_blocks):
        ...
        for ib in range(8):
            ...
            dscale = (d * (1 + 2 * ((scale_byte >> (4 * (ib % 2))) & 0x0F).to(tl.float16))).to(tl.float16)
            for il in range(4):
...
def ggml_gemm_iq3_s_triton(W: torch.Tensor, X: torch.Tensor, row: int) -> torch.Tensor:
