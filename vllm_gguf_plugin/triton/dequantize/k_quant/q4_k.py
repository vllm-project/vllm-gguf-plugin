...
from ...gemm.utils import GGML_TYPE_Q4_K, load_f16_from_u8
from ..utils import (
    dequant_offsets,
...
@triton.jit
def q4_k_dequantize_kernel(w_ptr, y_ptr, total, BLOCK_SIZE: tl.constexpr):
    offs, mask = dequant_offsets(total, BLOCK_SIZE)
    block_ptrs = w_ptr + (offs // 256) * 144
    pos = offs % 256
    group = pos // 64
    rem = pos % 64
    part = rem // 32
    ...
    out = q.to(tl.float32) * (scale_q.to(tl.float32) * dall)
    out = out - min_q.to(tl.float32) * dmin
    tl.store(y_ptr + offs, out, mask=mask)


def ggml_dequantize_q4_k_triton(
    W: torch.Tensor,
    m: int,
    n: int,
    dtype: torch.dtype | None = None,
) -> torch.Tensor:
    return run_dequantize_kernel(q4_k_dequantize_kernel, W, m, n, dtype, GGML_TYPE_Q4_K)
